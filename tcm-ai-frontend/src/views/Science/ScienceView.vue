<template>
  <div class="science-wrapper">
    <div class="science-nav-shell">
      ...
      <aside class="sidebar-nav-vertical" :class="{ collapsed: isSidebarCollapsed }">
        <button
        ...
        <button
          class="sidebar-btn"
          :class="{ collapsed: isSidebarCollapsed, active: activeTab === 'meridian' }"
          @click="setActiveTab('meridian')"
        >
          <i class="fa-solid fa-chart-line"></i>
          <span v-show="!isSidebarCollapsed">脉络图</span>
        </button>
        <button
          class="sidebar-btn"
          :class="{ collapsed: isSidebarCollapsed, active: activeTab === 'diagnosis' }"
          @click="setActiveTab('diagnosis')"
        >
          <i class="fa-solid fa-stethoscope"></i>
          <span v-show="!isSidebarCollapsed">四诊八脉</span>
        </button>
        <button
          class="sidebar-btn"
          :class="{ collapsed: isSidebarCollapsed, active: activeTab === 'history' }"
          @click="setActiveTab('history')"
        >
          <i class="fa-solid fa-book-open"></i>
          <span v-show="!isSidebarCollapsed">十大名医</span>
        </button>
      </aside>
    </div>
...
const setActiveTab = async (tab) => {
  ...
  // Ensure section switches always start from the page top.
...
.sidebar-nav-vertical.collapsed {
  width: 72px;
  padding: 24px 10px;
}
